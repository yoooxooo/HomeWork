...
public class TowerMainTest {
    public static void main(String[] args) throws InterruptedException {
                ...
                if (!rulesLock) {
                    ...
                    if ("Автоматический".equals(rules) || "Ручной".equals(rules)) {
                        if ("Автоматический".equals(rules)) {
                            ...
                            timer = (int) ((Double.parseDouble(console.nextLine().trim()) * 1000));
                            if (timer < 0) {
        ...
        TowerAutoTest game = new TowerAutoTest(pikeAmount, circlesAmount, timer);
        ...
        do {
            if ("Ручной".equals(rules)) {
                ...
                game.autoNext(circlesAmount, pikeAmount, timer);
            }
    ...
    public static boolean manualNext(int pikeAmount, TowerAutoTest game) {
        Scanner console = new Scanner(System.in);
        int input1, input2;
        String input;
        try {
            ...
            if (game.checkExit(input)) {
                System.out.println("Хочешь сохранить игру? Да/Нет");
                while (true) {
                    String save = console.nextLine().trim();
                    if ("Да".equals(save)) {
                        System.out.println("Введи название сохранения");
                        break;
                    }
                    if ("Нет".equals(save)) {
                        ...
                        System.out.println("Это не похоже ни на 'Да', ни на 'Нет'\nВведи еще раз");
                    }
                }
                return true;
            }
